...
from ...algo.utils import update_settings_from_args
from ...config import get_settings, global_settings
from ...git_providers.utils import apply_repo_settings
from ...lib.log import LoggingFormat, get_logger, setup_logger

setup_logger(fmt=LoggingFormat.JSON, level="DEBUG")
router = APIRouter(prefix="/v1", tags=["gitlab"])
...
async def _perform_commands_gitlab(commands_conf: str, api_url: str,
                                   log_context: dict):
    ...
    for command in commands:
        try:
            ...
            get_logger().info(f"Data context: {log_context}")
        except Exception as e:
...
async def gitlab_webhook(background_tasks: BackgroundTasks, request: Request):
    ...
    async def inner(data: dict):
        ...
        if data.get('object_kind') == 'merge_request' and data['object_attributes'].get('action') in ['open', 'reopen']:
            ...
            await _perform_commands_gitlab("pr_commands", url, log_context)
            ...
            try:
                ...
                await _perform_commands_gitlab("push_commands", url, log_context)
            except Exception as e:
